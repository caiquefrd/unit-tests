...
export const createContact = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!name || !phone) {
      res.status(400).json({ success: false, error: "Nome e telefone são obrigatórios." });
      return;
    }

    // Validation: name length
    if (typeof name !== 'string' || name.length < 3) {
      res.status(400).json({ success: false, error: "O nome deve ter pelo menos 3 caracteres." });
      return;
    }

    // Validation: phone format (only digits, 10-11 digits typical for Brazil)
    if (!/^\d{10,11}$/.test(phone)) {
      res.status(400).json({ success: false, error: "Formato de telefone inválido." });
      return;
    }
